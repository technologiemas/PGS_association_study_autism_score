...
# 3. creating factors from some relevant variables
data_long = data_long %>%
...
# rename to Mother and Father
data_long$rater_type = recode(data_long$rater_type,
                               "m12" = "Mother",
                               "v12" = "Father")

# 5. Create parent-child only subset and filter so both are present
data_long = data_long %>%
  filter(rater_type %in% c("Mother", "Father")) %>%
  group_by(FISNumber) %>%
  filter(n_distinct(rater_type) == 2) %>%
  ungroup()
...
rhs_m1 = paste(rhs_m0, "+ sex + rater_type + age")

rhs_m2 = paste(rhs_m0, "+ sex * rater_type + age")

rhs_m3 = paste(rhs_m0, "+ PGS_scaled * rater_type + PGS_scaled * sex + sex * rater_type + PLATFORM + age +",
               paste0("PC", 1:10, "_scaled", collapse = " + "))

rhs_m4 = paste(rhs_m0, "+ PGS_scaled * rater_type * sex + PLATFORM + age +",
               paste0("PC", 1:10, "_scaled", collapse = " + "))

rhs_m5 = paste(rhs_m0, "+ PGS_scaled * rater_type * sex + (PLATFORM + age +",
               paste0("PC", 1:10, "_scaled", collapse = " + "), 
